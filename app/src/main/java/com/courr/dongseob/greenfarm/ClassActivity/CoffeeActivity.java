...
public class CoffeeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.appcoffee).into(imageView1);

        PhotoView imageView2 = (PhotoView) findViewById(R.id.imageView2);
        Glide.with(this).load(R.drawable.map_capstone).into(imageView2);
        ...
        toolbar.setTitle("커피 쿠폰");

        DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("커피");
        ...
        bt_qring.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (point_value>=5){
                    ...
                    builder.setTitle("커피쿠폰")        // 제목 설정
                            .setMessage("한번 받으면 더이상 받을수 없습니다.*이 버튼은 부스운영자가 클릭합니다. ")        // 메세지 설정
                            .setCancelable(false)        // 뒤로 버튼 클릭시 취소 가능 설정
                            .setNeutralButton("확인", new DialogInterface.OnClickListener(){
                                // 확인 버튼 클릭시 설정
                                public void onClick(final DialogInterface dialog, int whichButton) {
                                    final DatabaseReference t_getPQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("커피");
                    ...
                    Toast.makeText(CoffeeActivity.this, "5포인트이상 획득가능 합니다.", Toast.LENGTH_SHORT).show();
                }
            }
        });
