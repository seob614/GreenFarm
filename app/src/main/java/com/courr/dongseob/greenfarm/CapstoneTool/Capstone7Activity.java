...
public class Capstone7Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_capstone7);
        ...
        toolbar.setTitle("구연산 함유 폐감귤을 활용한 토양 내 카드뮴 부동화 효과의 규명");
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("구연산 함유 폐감귤을 활용한 토양 내 카드뮴 부동화 효과의 규명")){
                        //수정
                                ...
                                alertDialogBuilderUserInput
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("3555810")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("캡스톤").child("구연산 함유 폐감귤을 활용한 토양 내 카드뮴 부동화 효과의 규명");

                                                    t_PQRreference.setValue("구연산 함유 폐감귤을 활용한 토양 내 카드뮴 부동화 효과의 규명");

                                                }
                                                else{
                                                    Toast.makeText(Capstone7Activity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(Capstone7Activity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("구연산 함유 폐감귤을 활용한 토양 내 카드뮴 부동화 효과의 규명")){

                    Toast.makeText(Capstone7Activity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(Capstone7Activity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
