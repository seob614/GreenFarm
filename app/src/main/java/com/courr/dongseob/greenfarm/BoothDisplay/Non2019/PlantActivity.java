...
public class PlantActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.bugflower2).into(imageView1);
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("식충식물 전시")){
                        //수정
                                ...
                                alertDialogBuilderUserInput
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("5751173126")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("선물").child("식충식물 전시");

                                                    t_PQRreference.setValue("식충식물 전시");


                                                }
                                                else{
                                                    Toast.makeText(PlantActivity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(PlantActivity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("식충식물 전시")){

                    Toast.makeText(PlantActivity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(PlantActivity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
