...
public class PictureActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.photographe).into(imageView1);

        PhotoView imageView2 = (PhotoView) findViewById(R.id.imageView2);
        Glide.with(this).load(R.drawable.map_picture).into(imageView2);
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("당신을 위해서라면")){
                        //수정
        ...
        DatabaseReference reservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("생명환경화학과").child("당신을 위해서라면");
        ...
        reservereference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (item.contains("시")) {
                    ...
                    DatabaseReference nownumreference = FirebaseDatabase.getInstance().getReference().child("예약").child("당신을 위해서라면").child(item.substring(0, 8)).child("대기");
        ...
        bt_reserve.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent reserve_intent = new Intent(PictureActivity.this , Reserve123456Activity.class);
                ...
                reserve_intent.putExtra("booth", "당신을 위해서라면");
                                ...
                                alertDialogBuilderUserInput
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("65746479")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("선물").child("당신을 위해서라면");

                                                    t_PQRreference.setValue("당신을 위해서라면");

                                                    final DatabaseReference myreservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("생명환경화학과").child("당신을 위해서라면");
                                                    ...
                                                    myreservereference.addValueEventListener(new ValueEventListener() {
                                                        ...
                                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                            if (dataSnapshot.getValue().toString().contains("예약대기중")){
                                                                ...
                                                                DatabaseReference numPQRreference = FirebaseDatabase.getInstance().getReference().child("예약").child("당신을 위해서라면").child(dataSnapshot.getValue().toString().substring(0,8)).child("대기");
                                                    ...
                                                    Toast.makeText(PictureActivity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(PictureActivity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("당신을 위해서라면")){

                    Toast.makeText(PictureActivity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(PictureActivity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
