...
public class Capstone4Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_capstone4);
        ...
        toolbar.setTitle("국내산 도라지 가격결정요인분석");
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("국내산 도라지 가격결정요인분석")){
                        //수정
                                ...
                                alertDialogBuilderUserInput
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("46310913")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("캡스톤").child("국내산 도라지 가격결정요인분석");

                                                    t_PQRreference.setValue("국내산 도라지 가격결정요인분석");

                                                }
                                                else{
                                                    Toast.makeText(Capstone4Activity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(Capstone4Activity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("국내산 도라지 가격결정요인분석")){

                    Toast.makeText(Capstone4Activity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(Capstone4Activity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
