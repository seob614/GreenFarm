...
public class JuiceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.juice2).into(imageView1);
        ...
        toolbar.setTitle("에너지바,무엇이든지 통조림");
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("에너지바,무엇이든지 통조림")){
                        //수정
        ...
        DatabaseReference reservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("식품공학과").child("에너지바,무엇이든지 통조림");
        ...
        reservereference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (item.contains("시")) {
                    ...
                    DatabaseReference nownumreference = FirebaseDatabase.getInstance().getReference().child("예약").child("에너지바,무엇이든지 통조림").child(item.substring(0, 8)).child("대기");
        ...
        bt_reserve.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent reserve_intent = new Intent(JuiceActivity.this , ReserveActivity.class);
                ...
                reserve_intent.putExtra("booth", "에너지바,무엇이든지 통조림");
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("212333356")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("선물").child("에너지바,무엇이든지 통조림");

                                                    t_PQRreference.setValue("에너지바,무엇이든지 통조림");

                                                    final DatabaseReference myreservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("식품공학과").child("에너지바,무엇이든지 통조림");
                                                    ...
                                                    myreservereference.addValueEventListener(new ValueEventListener() {
                                                        ...
                                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                            if (dataSnapshot.getValue().toString().contains("예약대기중")){
                                                                ...
                                                                DatabaseReference numPQRreference = FirebaseDatabase.getInstance().getReference().child("예약").child("에너지바,무엇이든지 통조림").child(dataSnapshot.getValue().toString().substring(0,8)).child("대기");
                                                    ...
                                                    Toast.makeText(JuiceActivity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(JuiceActivity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("에너지바,무엇이든지 통조림")){

                    Toast.makeText(JuiceActivity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(JuiceActivity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
