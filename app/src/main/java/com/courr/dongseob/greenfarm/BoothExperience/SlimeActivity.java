...
public class SlimeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.slime2).into(imageView1);
        ...
        toolbar.setTitle("내 친구 슬라임");

        final DatabaseReference getpointreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child("평점").child("점수");
        ...
        getpointreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                final DatabaseReference getnumreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child("평점").child("인원");
        ...
        t_PQRreference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                while (i.hasNext()) {
                    if (((DataSnapshot) i.next()).getKey().contains("내 친구 슬라임")){
                        //수정
        ...
        DatabaseReference reservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("바이오산업기계공학과").child("내 친구 슬라임");
        ...
        reservereference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (item.contains("시")) {
                    ...
                    DatabaseReference nownumreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child(item.substring(0, 8)).child("대기");
        ...
        bt_reserve.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent reserve_intent = new Intent(SlimeActivity.this , Reserve123456Activity.class);
                reserve_intent.putExtra("studentnum", studentnum);
                reserve_intent.putExtra("name", name);
                reserve_intent.putExtra("token", token);
                reserve_intent.putExtra("department", "바이오산업기계공학과");
                reserve_intent.putExtra("booth", "내 친구 슬라임");
                reserve_intent.putExtra("havenum","2");
                                ...
                                alertDialogBuilderUserInput
                                        ...
                                        .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialogBox, int id) {
                                                ...
                                                if (InputDialogEditText.getText().toString().contains("561185654109")){
                                                    DatabaseReference t_PQRreference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("QR코드").child("선물").child("내 친구 슬라임");

                                                    t_PQRreference.setValue("내 친구 슬라임");

                                                    final DatabaseReference myreservereference = FirebaseDatabase.getInstance().getReference().child("토큰").child(token).child("예약").child("바이오산업기계공학과").child("내 친구 슬라임");
                                                    ...
                                                    myreservereference.addValueEventListener(new ValueEventListener() {
                                                        ...
                                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                            if (dataSnapshot.getValue().toString().contains("예약대기중")){
                                                                ...
                                                                DatabaseReference numPQRreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child(dataSnapshot.getValue().toString().substring(0,8)).child("대기");
                                                    ...
                                                    builder.setItems(R.array.point, new DialogInterface.OnClickListener(){
                                                        ...
                                                        public void onClick(DialogInterface dialog, final int pos)
                                                        {
                                                            ...
                                                            final DatabaseReference numreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child("평점").child("인원");
                                                            ...
                                                            final DatabaseReference pointreference = FirebaseDatabase.getInstance().getReference().child("예약").child("내 친구 슬라임").child("평점").child("점수");
                                                    ...
                                                    Toast.makeText(SlimeActivity.this, "번호를 정확히 입력하십시오.", Toast.LENGTH_LONG).show();
                                                }
                                            }
                                        })
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result!=null) {
            if (result.getContents() == null) {
                Toast.makeText(SlimeActivity.this, "취소!", Toast.LENGTH_SHORT).show();
            } else {

                String QR = result.getContents();

                if (QR.contains("내 친구 슬라임")){

                    Toast.makeText(SlimeActivity.this, "스캔완료!", Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(SlimeActivity.this, "해당부스의 QR코드가 아닙니다!.", Toast.LENGTH_SHORT).show();
                }
